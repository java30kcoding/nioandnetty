...
public class BufferDemo {
    ...
    public static void main(String[] args) {
        ...
        //转换为读取模式(不调用flip方法，也是可以读取数据的，但是position记录的位置不对)
        ...
        //继续写入3个字节，此时读取模式下，limit=3，position=2，继续写入只能覆盖写入一条数据
        //clear()方法清除整个缓冲区。compact()方法仅清除已阅读的数据。转换为写入模式
        byteBuffer.compact();
        byteBuffer.put((byte)3);
        byteBuffer.put((byte)4);
        byteBuffer.put((byte)5);
        System.out.println(String.format("最后情况：capacity容量：%s，position位置：%s，limit限制：%s",
                byteBuffer.capacity(), byteBuffer.position(),byteBuffer.limit()));
